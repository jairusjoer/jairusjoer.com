...
`contrast-color()` ensures minimum contrast in accordance with WCAG AA[^wcag] and returns either black or white based on the provided input. In this demonstration, the theme colour picker sets the CSS variable `--color-theme-raw`, which can then be used to derive a contrasting foreground colour:

<figure>

```css
:root {
  --color-theme-contrast: light-dark(white, black);

  @supports (color: contrast-color(red)) {
    --color-theme-contrast: contrast-color(var(--color-theme));
  }
}
```

<figcaption>Derive `contrast-colour()` from the theme, with a widely supported fallback.</figcaption>
</figure>
...
```css
:root {
  --color-theme: light-dark(oklch(from var(--color-theme-raw) 0.5 c h), oklch(from var(--color-theme-raw) 0.75 c h));
}
```

<figcaption>Stabilise perceptual lightness across light and dark modes using <code>oklch()</code>.</figcaption>
</figure>
